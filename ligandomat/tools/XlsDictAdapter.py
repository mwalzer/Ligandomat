...
def XlsDictWriter(f, ld, sheet_name='sheet', headerlist=[]):
    ...
    if len(ld) > 0:
        ...
        for i, key in enumerate(headerlist):
            # using smart_str to avoid ascii encoding problems! (If for example Delta is in header)
            sheet.write(0, i, label=smart_str(key))
